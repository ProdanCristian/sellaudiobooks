...
export default function RichEditor({ value, onChange, onSave, isSaving, className, autoSave = false }: RichEditorProps) {
    ...
    const [, setEditorState] = useState(0) // Force re-render when buttons are clicked
    ...
    const applyTransform = async (action: TransformAction) => {
        ...
        try {
            ...
            console.log('Transform request:', { action, text: selected })
            const response = await fetch('/api/writing-assistant/transform', {
                method: 'POST',
    ...
    return (
        <div className={className}>
            <div className="flex flex-wrap gap-1 mb-2">
                <Button type="button" variant={editor.isActive('bold') ? 'default' : 'outline'} size="sm" onMouseDown={(e) => e.preventDefault()} onClick={() => {
                    editor.chain().focus().toggleBold().run()
                    setEditorState(prev => prev + 1) // Force immediate re-render
                }} disabled={!editor.can().chain().focus().toggleBold().run()}>
                    <Bold className="h-4 w-4" />
                </Button>
                <Button type="button" variant={editor.isActive('italic') ? 'default' : 'outline'} size="sm" onMouseDown={(e) => e.preventDefault()} onClick={() => {
                    editor.chain().focus().toggleItalic().run()
                    setEditorState(prev => prev + 1) // Force immediate re-render
                }} disabled={!editor.can().chain().focus().toggleItalic().run()}>
                    <Italic className="h-4 w-4" />
                </Button>
                <Button type="button" variant={editor.isActive('heading', { level: 1 }) ? 'default' : 'outline'} size="sm" onMouseDown={(e) => e.preventDefault()} onClick={() => {
                    const { from, to } = editor.state.selection
                ...
                <Button type="button" variant={editor.isActive('heading', { level: 2 }) ? 'default' : 'outline'} size="sm" onMouseDown={(e) => e.preventDefault()} onClick={() => {
                    const { from, to } = editor.state.selection
