  ...
  try {
    ...
    const submitResponse = await fetch(
      "https://api.wavespeed.ai/api/v3/google/imagen4",
      {
        ...
        body: JSON.stringify({
          ...
          } Do not include any other text or elements apart from the title. Only show the book cover design, no book mockup.`,
          aspect_ratio: "3:4",
          num_images: 1,
          negative_prompt:
            "blurry, low quality, distorted text, book mockup, 3D book, physical book, additional text, author name, publisher name, book spine",
    ...
    if (bookId) {
      try {
        ...
        console.warn(
          "Failed to delete old cover, continuing with generation:",
          error
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "R2 upload failed, falling back to original URL. Error:",
        r2Error
      );
      ...
      return NextResponse.json({
        ...
        fallbackUsed: true,
        message:
          "Cover generated successfully (using temporary URL - R2 upload failed)",
      });
    }
  } catch (error) {
